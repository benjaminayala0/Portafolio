...
const SERVICES = [
    ...
    { label: 'Mercado Pago', desc: 'Cobros & Webhooks', Icon: ShieldCheck, color: 'text-cyan-400' },
];
...
const VisuBookEcosystem = () => {
    ...
    return (
        <div className="space-y-6">
            <h4 className="text-xs font-mono text-text-primary uppercase tracking-wider mb-4 text-center opacity-80">
                {t('studies.ecosystem')}
            </h4>
            ...
            <div className="space-y-3">
                {FLOW_NODES.map((node, i) => (
                    <React.Fragment key={node.id}>
                        <div className={`relative group/node rounded-xl border ${node.borderColor} bg-surface/50 p-4 transition-all hover:bg-surface-lighter/30`}>
                            ...
                            <div className="relative z-10 flex items-center gap-3.5">
                                <div className="p-2 bg-surface rounded-lg border border-white/5 shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-0.5 sm:gap-2 mb-1">
                                        ...
                                        <span className="text-text-secondary/60 font-mono text-[10px]">{node.tech}</span>
                                    </div>
                                    <div className="text-[11px] font-mono text-text-secondary/80">
                                        {node.features.map(f => t(f)).join(' · ')}
                                    </div>
                                </div>
                            </div>
                        </div>
                        {i < FLOW_NODES.length - 1 && (
                            <div className="flex justify-center">
                                <ArrowRight size={14} className="text-text-secondary/20 rotate-90" />
                            </div>
                        )}
                    </React.Fragment>
                ))}
            ...
            <div className="mt-6 pt-4 border-t border-text-secondary/10">
                ...
                <div className="grid grid-cols-3 gap-2.5">
                    {SERVICES.map(svc => (
                        <div key={svc.label} className="flex flex-col items-center gap-1.5 p-3 rounded-lg bg-surface/30 border border-white/5 text-center hover:border-white/10 transition-colors">
                            <svc.Icon size={16} className={svc.color} />
                            <span className="text-[11px] font-medium text-text-primary">{t(svc.label)}</span>
                            <span className="text-[10px] text-text-secondary/80 font-mono leading-tight">{t(svc.desc)}</span>
                        </div>
                    ))}
                </div>
            </div>
